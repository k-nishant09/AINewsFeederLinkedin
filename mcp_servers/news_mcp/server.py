...
────────────────────────────
  GNews free plan allows 1 request/second.
  Set GNEWS_REQUEST_DELAY_MS=1100 (default) to stay within the limit
  when the workflow issues multiple sequential category searches.
...
# ── GNews config ──────────────────────────────────────────────────────────────
# Injected from OpenShift Secret / .env
...
async def _gnews_search(
    ...
    if not GNEWS_API_KEY:
        logger.warning("GNEWS_API_KEY not set — returning mock results for dev")
...
def health():
    return {
        ...
        "server":           "news-mcp",
        "provider":         "gnews",
        "api_key_set":      bool(GNEWS_API_KEY),
